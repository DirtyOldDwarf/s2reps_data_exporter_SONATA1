...
pids_sonata1 = {
    '301':['2','1'],
    '302':['2','1'],'303':['2','2'],'304':['2','1'],'305':['2','1'],'306':['2','2'],
    '307':['2','2'],'308':['2','2'],'309':['2','2'],'310':['2','2'],'312':['2','1'],
    '314':['2','1'],'315':['2','1'],'316':['2','2'],'317':['2','2'],'318':['2','1'],
    '319':['2','2'],'320':['2','2'],'321':['2','2'],'322':['2','2'],'323':['2','2'],
    '324':['2','2'],'325':['2','1'],'326':['2','2'],'327':['2','1'],'328':['2','1'],
    '329':['2','1'],'330':['2','2'],'332':['1','1'],'334':['2','1'],'335':['1','2'],
    '336':['2','2'],'338':['1','2'],'339':['1','2'],'341':['1','2'],'342':['1','2'],
    '343':['1','2'],'344':['1','1'],'345':['1','1'],'346':['1','2'],'347':['1','2'],
    '349':['1','2'],'350':['1','2'],'351':['1','1'],'352':['1','2'],'353':['1','1'],
    '354':['1','1'],'355':['1','1'],'356':['1','1'],'357':['1','1'],'358':['1','1'],
    '359':['1','1'],'360':['1','1'],'361':['2','1'],'362':['1','1'],'363':['1','1'],
    '364':['1','2'],'365':['1','2'],'366':['1','2'],'368':['2','2'],'370':['2','1'],
}

pids_sonata2 = {
# SONATA2
    '3002':['3','1'],'3004':['3','1'],'3005':['3','1'],'3006':['3','2'],'3008':['3','2'],
    '3011':['3','2'],'3012':['3','2'],'3013':['3','2'],'3014':['3','2'],'3015':['3','2'],
    '3016':['3','2'],'3017':['3','2'],'3018':['3','1'],'3020':['3','2'],'3021':['3','1'],
    '3022':['3','2'],'3023':['3','1'],'3024':['3','2'],'3025':['3','2'],'3026':['3','1']
}
...
db = client.sc2reps
collection = db.basic_sonata1_rev3
pids = pids_sonata1
...
for pid in pids:
    ...
    winLossCount = list(collection.aggregate(pipeline_winLossRatio))
    ...
    pipeline_difficultyLevels = [
        ...
        {"$group": {
            ...
            "ara": {"$avg": "$averageResourcesAvailable"},
            "arcr": {"$avg": "$averageResourcesCollectionRate"}
        }}
    ]

    difficultyLevelsCount = list(collection.aggregate(pipeline_difficultyLevels))
    ...
    for lvl in difficultyLevels:
        ...
        if len(tmpDiffLvlCount) > 0:
            ...
            # Calculate Spending Quotient 35*(0.00137*ARCR-LN(ARA))+240
            if tmpDiffLvlCount['arcr'] > 600:
                output['trainingMatchesLvl_SQ_' + lvl.replace(' ', '_').replace('(', '').replace(')', '')] = 35 * (
                0.00137 * tmpDiffLvlCount['arcr'] - math.log(tmpDiffLvlCount['ara'])) + 265
            else:
                output['trainingMatchesLvl_SQ_' + lvl.replace(' ', '_').replace('(', '').replace(')', '')] = None
        else:
            output['trainingMatchesLvl_Count_' + lvl.replace(' ', '_').replace('(','').replace(')','')] = None
            output['trainingMatchesLvl_Time_' + lvl.replace(' ', '_').replace('(','').replace(')','')] = None
            output['trainingMatchesLvl_SQ_' + lvl.replace(' ', '_').replace('(','').replace(')','')] = None
    ...
    output['trainingTime'] = list(collection.aggregate(pipeline_trainingTime))[0]['total']
    ...
    qHotkeyUseCounter = 0
    qHotkeySetCounter = 0
    qHotkeyUseIntensityCounter = 0
    qHotkeySetIntensityCounter = 0
    ...
    for idx, match in enumerate(matchesForPId):
        ...
        if qTimeCounter == 0 or qTimeCounter <= quarterTime:
            ...
            qHotkeyUseCounter += match['hotkeysUsageTotal'] if 'hotkeysUsageTotal' in match else 0
            qHotkeySetCounter += match['hotkeysSetaddTotal'] if 'hotkeysSetaddTotal' in match else 0
            qHotkeyUseIntensityCounter += match['hotkeysUsageIntensity'] if 'hotkeysUsageIntensity' in match else 0
            qHotkeySetIntensityCounter += match['hotkeysSetaddIntensity'] if 'hotkeysSetaddIntensity' in match else 0
...
#                    qCounters[var_name][1] += 0 if match[var_name] == None else 1
        ...
        if (qTimeCounter >= quarterTime and qCount < 3) or idx == matchesForPId.count() - 1:
            ...
            output['hotkeysUse'+postfixDataKeys[qCount]] = qHotkeyUseCounter / qMatchesCounter
            output['hotkeysSet'+postfixDataKeys[qCount]] = qHotkeySetCounter / qMatchesCounter
            output['hotkeysUseIntensity'+postfixDataKeys[qCount]] = qHotkeyUseIntensityCounter / qMatchesCounter
            output['hotkeysSetIntensity'+postfixDataKeys[qCount]] = qHotkeySetIntensityCounter / qMatchesCounter
...
with open('sc2reps_aggregates_from-'+collection.name+'-at-'+str(datetime.now()).replace("-","").replace(":","").replace(" ","")[:14]+'.csv', 'wb') as output_file:
